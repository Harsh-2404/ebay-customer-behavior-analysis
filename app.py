...
# Textwrap use karke syntax error ko hamesha ke liye block kiya hai
...
# ==========================================
# 2. SAFE DATA LOADING & DUPLICATE CLEANUP
# ==========================================
@st.cache_data
def load_data():
    try:
    ...
    # Space cleanup & Duplicates Removal
    ...
    def get_1d_series(dataframe, column_name):
        data = dataframe[column_name]
        if isinstance(data, pd.DataFrame):
            return data.iloc[:, 0]
        return data

    df["age"] = pd.to_numeric(get_1d_series(df, "age"), errors="coerce")
    df = df[(df["age"] >= 13) & (df["age"] <= 80)].copy()

    df["Shopping_Satisfaction"] = pd.to_numeric(
        get_1d_series(df, "Shopping_Satisfaction"), errors="coerce"
    ).fillna(3)
...
with tab4:
    st.dataframe(df_filtered, use_container_width=True)    color: #555555;
    margin-bottom: 4px;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
...
def load_data():
    ...
    # 1. Clean space from column headers
    df.columns = df.columns.str.strip()

    # 2. Drop duplicate columns upfront
    df = df.loc[:, ~df.columns.duplicated(keep="first")].copy()

    # 3. Safe mapping to unified target names
    col_map = {}
    seen_mapped = set()
    ...
    # 4. Drop duplicates again after renaming to prevent duplicate mapped names
    df = df.loc[:, ~df.columns.duplicated(keep="first")].copy()
    ...
    # Explicit slice to guarantee 1D series
    df = df[keep_cols].copy()

    # Helper function to extract guaranteed 1D Pandas Series
    def extract_series(dataframe, column_name):
        data = dataframe[column_name]
        if isinstance(data, pd.DataFrame):
            return data.iloc[:, 0]
        return data
    ...
    # Type Conversions
    df["age"] = pd.to_numeric(extract_series(df, "age"), errors="coerce")
    df = df[(df["age"] >= 13) & (df["age"] <= 80)].copy()

    df["Shopping_Satisfaction"] = pd.to_numeric(
        extract_series(df, "Shopping_Satisfaction"), errors="coerce"
    ).fillna(3)
...
# ==========================================
# 2. SAFE DATA LOADING & DEDUPLICATION
# ==========================================
@st.cache_data
def load_data():
    try:
